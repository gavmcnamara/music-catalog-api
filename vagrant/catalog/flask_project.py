...
def genreItem(genre_id):
    ...
    items = session.query(Band).filter_by(
        genre_id=genre_id).all()
    return render_template('band.html', items=items, genre=genre)
...
def newGenreItem(genre_id):
    ...
    if request.method == 'POST':
        newItem = Band(name=request.form['name'], description=request.form[
                           'description'], year=request.form['year'], genre_id=genre_id)
        session.add(newItem)
        session.commit()
...
def editGenreItem(genre_id, band_id):
    ...
    if request.method == 'POST':
        ...
        return render_template(
            'editgenreitem.html', genre_id=genre_id, band_id=band_id, item=editedItem)
